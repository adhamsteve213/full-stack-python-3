...
export default function Tops() {
  ...
  const { isInWishlist, toggleWishlist } = useContext(WishlistContext);
    useEffect(() => {
        fetch('http://127.0.0.1:8000/tops/')
    ...
    <Grid container spacing={2} sx={{ padding: 2 }}>
      {products.map((product) => (
        <Grid item key={product.id} xs={12} sm={6} md={4} lg={3}>
          <Link to={`/product/tops/${product.id}`} style={{ textDecoration: 'none' }}>
          <Card>
            <CardActionArea>
              <CardMedia
                component="img"
              ...
              <CardContent>
                ...
                if (cartItem) {
                  ...
                  return (
                    <Button onClick={(event) => { event.stopPropagation(); addToCart(product, 1, 'tops'); }} style={{ backgroundColor:'black',color:'white', padding:'10px',margin:'20px',borderRadius: '10px'}} sx={{ mt: 1 }}>
